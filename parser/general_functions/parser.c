...
/*   Updated: 2022/02/04 15:40:29 by cproesch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

int	review_operators(t_data *data, char **token)
{
	int	i;
	...
	while (token[i])
	{
		...
		|| (token[i][0] == '<'))
		{
			if ((ft_strncmp(token[i], "|\0", 2) 
				...
				return (ft_error2("Syntax error", data, 2));
		}
		i++;
	}
	return (1);
...
{
	if (!review_operators(data, token))
		return (0);
